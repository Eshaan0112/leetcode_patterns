...
paths that are chosen to be included and not to be included. For eg, let's say input = [1,2,1] and we're making subsets
    ...
    We need to make it so that once we've traversed the choice-to-include using duplicate elements, we make sure we skip the duplicates before we start our traversal
    down the path of choice-not-to-include. In the code, this should be done after popping back up (check subsets 2 problem on Neetcode.io)
